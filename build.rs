...
fn main() {
 println!("cargo:warning=running");
    ...
    println!("cargo:warning=copying assets to build dir {:?}", out_dir_buff);
   ...
   let build_dir = out_dir_buff.to_str().unwrap();
    ...
    Command::new("cp").args(&["-r", &assets[..], build_dir]).status().unwrap();
    Command::new("cp").args(&["-r", &config[..], build_dir]).status().unwrap();
    Command::new("cp").args(&["-r", &levels[..], build_dir]).status().unwrap();
